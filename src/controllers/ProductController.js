...
module.exports = {
	...
	async getSearch(req, res) {
		...
		if (page) {
			my_query = `select id, product_name, price, image_url from products WHERE product_name ~* '^(?=.*${value})' LIMIT 25 OFFSET ${limit}`;
		} else {
			my_query = `select id, product_name, price, image_url from products WHERE product_name ~* '^(?=.*${value})' LIMIT 5`;
		}
	...
	async getSection(req, res) {
		...
		if (Object.keys(query).length === 0 && query.constructor === Object) {
			my_query = 'SELECT id, product_name, price, image_url FROM products WHERE section = ? LIMIT 20';
		} else {
			my_query = `SELECT id, product_name, price, image_url FROM products WHERE section = ? LIMIT 25 OFFSET ${limit}`;
		}
